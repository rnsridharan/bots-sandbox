...
bot.dialog('greeting',  [
    function (session) {
    	...
    	builder.Prompts.text(session, session.gettext('greeting'), 
         		{ speak: speak(session, 'greeting'),
         		  retrySpeak: speak(session, 'greeting'),
         		  inputHint: builder.InputHint.ignoringInput 
         		});
    	builder.Prompts.text(session, session.gettext('instructions'), 
         		{ speak: speak(session, 'instructions'),
         		  retrySpeak: speak(session, 'instructions'),
         		  inputHint: builder.InputHint.ignoringInput 
         		});
    	
    	session.say(session.gettext('greeting'), 
    			{speak: speak(session, 'greeting'),
    		inputHint: builder.InputHint.ignoringInput 
    		});
    	
    	session.say(session.gettext('instructions'), 
    			{speak: speak(session, 'instructions'),
    		inputHint: builder.InputHint.ignoringInput 
    		});
